...
| Text-to-speech   | EdgeTTS (`en-US-GuyNeural` + `en-US-AriaNeural`) for mock consultation audio           |
...
└─────────────┘             │ ── triage agent  │         └──────────────────┘
       ...
       │                    │ ── tts + asr     │         └──────────────────┘
       │ Reminders panel    │                  │ ──────▶ ┌──────────────────┐
       │ Transcript widget  │                  │         │  EdgeTTS (local) │
       │                    │                  │         └──────────────────┘
       │                    │                  │ ──────▶ ┌──────────────────┐
       │                    │                  │         │  Speechmatics    │
       │                    │                  │         │  batch ASR       │
...
## The transcription pipeline (Speechmatics + EdgeTTS)
...
1. Picks a dialogue scenario based on the ticket state:
   ...
   - `general` otherwise
2. Calls EdgeTTS with two voices to synthesize an MP3 of the doctor–patient turns
...
apps/
  api/                    FastAPI service
    app/
      ...
      integrations/       Featherless, Speechmatics, EdgeTTS clients
...
6. Click "▶ Play & transcribe" on Bruce's ticket detail. EdgeTTS synthesizes the doctor's first-contact dialogue; Speechmatics returns a speaker-diarized transcript in ~10 seconds.
...
| POST   | `/api/admin/tickets/{id}/transcript`              | `X-Admin-Password`    | EdgeTTS → Speechmatics pipeline for this ticket     |
...
- Audio synthesis by [EdgeTTS](https://github.com/rany2/edge-tts)
